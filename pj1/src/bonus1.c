/* =================================== */
/* bonus 1: Two servers and two queues */
/* =================================== */

#include "utils.h"

server S[2];
customer *queue[2][10000];
int qs[2], qe[2];
...
int cust_waiting(int i){
    if(qe[i] >= qs[i])
        return qe[i] - qs[i];
    return (qe[i] + 10000 - qs[i]);
}

void push(customer *c){
    int Q = 0;
    if(cust_waiting(0) > cust_waiting(1))
        Q = 1;
    if((qe[Q] == qs[Q]) && (queue[Q][qs[Q]] != NULL)){
...
int main(int argc, char* argv[]){
    ...
    while(1){
        if((cust_waiting(0)==0) || (cust_waiting(1)==0)){
